...
public class Interpreter implements ALU {
	...
	public void read(Program p, short inst) throws MemoryException {
		switch (inst >> 12 & 0xf) {
		case 0:
			branch(p, inst >> 9 & 0x7, inst & 0x1ff);
			break;
		case 1:
			if ((inst & 0x20) == 0)
			...
			break;
		case 2:
			load(p, inst >> 9 & 0x7, inst & 0x1ff);
			break;
		case 3:
			store(p, inst >> 9 & 0x7, inst & 0x1ff);
			break;
		case 4:
			jumpsub(p, (inst & 0x400) != 0, inst & 0x1ff);
			break;
		case 5:
			if ((inst & 0x20) == 0)
			...
			break;
		case 6:
			loadR(p, inst >> 9 & 0x7, inst >> 6 & 0x7, inst & 0x3f);
			break;
		case 7:
			storeR(p, inst >> 9 & 0x7, inst >> 6 & 0x7, inst & 0x3f);
			break;
		case 8:
			debug(p);
			break;
		case 9:
			not(p, inst >> 9 & 0x7, inst >> 6 & 0x7);
			break;
		case 10:
			loadI(p, inst >> 9 & 0x7, inst & 0x1ff);
			break;
		case 11:
			storeI(p, inst >> 9 & 0x7, inst & 0x1ff);
			break;
		case 12:
			jumpsubR(p, (inst & 0x400) != 0, inst >> 6 & 0x7, inst & 0x3f);
			break;
		case 13:
			ret(p);
			break;
		case 14:
			loadEA(p, inst >> 9 & 0x7, inst & 0x1ff);
			break;
		case 15:
			trap(p, inst & 0xff);
			break;
	...
	public void branch(Program p, int nzp, int pgoff) {
		if ((nzp & 4) != 0 && m.r.n || (nzp & 2) != 0 && m.r.z
				|| (nzp & 1) != 0 && m.r.p)
	...
	public void debug(Program p) {
		...
		for (int i = 0; i < 8; i++)
			m.sys.print("R" + i + " "
					+ Integer.toHexString(m.r.r[i] + 0x10000).substring(1)
					+ "\n");
		if (m.r.n)
			m.sys.print("n");
		if (m.r.z)
	...
	public void load(Program p, int dr, int pgoff) throws MemoryException {
		m.r.r[dr] = p.getMemory((short) ((m.r.pc & 0xfe00) + pgoff));
		m.r.n = m.r.r[dr] < 0;
		m.r.z = m.r.r[dr] == 0;
		m.r.p = m.r.r[dr] > 0;
	}
	...
	public void loadI(Program p, int dr, int pgoff) throws MemoryException {
		m.r.r[dr] = p.getMemory(p.getMemory((short) ((m.r.pc & 0xfe00) + pgoff)));
		m.r.n = m.r.r[dr] < 0;
		m.r.z = m.r.r[dr] == 0;
		m.r.p = m.r.r[dr] > 0;
	}

	public void loadR(Program p, int dr, int br, int index)
			throws MemoryException {
		m.r.r[dr] = p.getMemory((short) (m.r.r[br] + index));
		m.r.n = m.r.r[dr] < 0;
		m.r.z = m.r.r[dr] == 0;
		m.r.p = m.r.r[dr] > 0;
	}
	...
	public void store(Program p, int sr, int pgoff) throws MemoryException {
		p.setMemory((short) ((m.r.pc & 0xfe00) + pgoff), m.r.r[sr]);
	}

	public void storeI(Program p, int sr, int pgoff) throws MemoryException {
		p.setMemory(p.getMemory((short) ((m.r.pc & 0xfe00) + pgoff)), m.r.r[sr]);
	}

	public void storeR(Program p, int sr, int br, int index)
			throws MemoryException {
		p.setMemory((short) (m.r.r[br] + index), m.r.r[sr]);
	}
	...
	public void trap(Program p, int vector) {
		...
		case 0x43: // RND: store a random number in R0
			...
			break;
		default:
			m.sys.error("Warning: unknown trap vector 0x"
