...
public class Console {
    ...
    private void help(final String... words) {
        ...
        if (words.length < 2 || words[1].length() == 0)
            ...
            m.ui.print(" Syntax: load [-iHEX] [<file>...]\n"
                ...
                + " parameter is omitted, it reloads the last loaded program.\n\n"
                + " The loader is also capable of loading multiple programs together into\n"
                + " memory. These files may be given together in one command, or spread out\n"
                + " over several load commands. If a file is loaded without the linked\n"
                + " libraries also being given, the command will list all the unlinked\n"
                + " symbols. If all external symbols are found in the given set of files,\n"
                + " the program will automatically link, upon which point the program can\n"
                + " be executed as above. The program will load all files sequentially into\n"
                + " memory starting from the initial program load address (IPLA), which can\n"
                + " be set using the -i flag and defaults to 0.\n\n"
                + "   > load test.o           loads 'test.o' from current directory\n"
                + "   > load test2.o test3.o  links 'test2.o' and 'test3.o' with 'test.o'\n"
                + "                           (assuming test references test 2 and 3)\n"
                + "   > load -i2000 test4.o   links 'test4.o' and sets the IPLA to 0x2000\n"
                + "   > load                  reloads 'test<1-4>.o' (assuming this was a\n"
                + "                           successful link)\n"
                + "   > load /usr/bin/test    loads 'test' from /usr/bin/\n\n"
                + " The file(s) must be in MMXI file format. Otherwise, any parsing errors\n"
